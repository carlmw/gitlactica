define(['vendor/three', 'vendor/TWEEN', 'planet', 'ship', 'course', 'raf'], function (THREE, TWEEN, Planet, Ship, Course, raf) {
  "use strict";

  return function (window, domContainer) {
    var distance = 1000,
        w = window.innerWidth,
        h = window.innerHeight,
        camera,
        scene,
        ship,
        planet,
        planet2,
        renderer;

    camera = new THREE.PerspectiveCamera(30, w / h, 0.1, 10000);
    camera.position.z = distance * 10;
    scene = new THREE.Scene();

    renderer = new THREE.WebGLRenderer({ antialias: true });
    renderer.setSize(w, h);

    registerHandlers(window);

    domContainer.appendChild(renderer.domElement);

    ship = new Ship(scene);
    planet = new Planet(scene);
    planet2 = new Planet(scene);
    planet.mesh.position.x -= 3000;
    planet2.mesh.position.x += 3000;

    ship.pivot.position.x = 3000;

    animate();

    function resizeHandler () {
      w = window.innerWidth;
      h = window.innerHeight;
      camera.aspect = w / h;
      camera.updateProjectionMatrix();
      renderer.setSize(w, h);
    }

    function render () {
      renderer.render(scene, camera);
    }

    function registerHandlers (window) {
      window.addEventListener('resize', resizeHandler);
    }

    function animate () {
      raf(animate);
      TWEEN.update();
      render();
    }
  };
});
